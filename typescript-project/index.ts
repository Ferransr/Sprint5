...
const weather = async () => {
  try {
    const respuesta = await fetch('https://api.openweathermap.org/data/2.5/weather?lat=41.3851&lon=2.1734&units=metric&appid=ee11567202766ed1b15e51654dcbf58e');
    const datos = await respuesta.json();
...
function cambiarImagenDeFondo(): void {
  ...
  const indiceAleatorio: number = Math.floor(Math.random() * imagenesDeFondo.length);
